...
import { getFactsInfo } from "../providers.js";
...
function buildTrackList(sorted: RawTrack[], station?: Station | null): TrackInfo[] {
...
export const rmfProvider: Provider = {
  ...
  parse(data: unknown, station?: Station | null): PlaylistResult | null {
    ...
    const processed = buildTrackList(sorted, station);

    return {
      current: resolveActiveTrack(processed, sorted, station, nowSec),
      all: processed,
    };
  },
};
