...
function classifyHistoryItems(filtered: TrackInfo[], nowSec: number): ClassifiedItem[] {
...
function buildHistorySignature(renderedItems: ClassifiedItem[]): string {
...
interface TargetItem extends ClassifiedItem {
  statusCls: string;
  key: string;
}

function reconcileHistoryDOM(targetItems: TargetItem[]): void {
...
function animateHistoryHeight(isPanelOpen: boolean, prevHeight: number): void {
  if (!isPanelOpen || prevHeight <= 0) return;

  const newHeight = els.historyList.getBoundingClientRect().height;
  if (newHeight <= 0 || Math.abs(prevHeight - newHeight) <= 2) return;
...
export function updateHistoryUI(animateSlideIn = false): void {
  ...
  const targetItems: TargetItem[] = renderedItems.map((item) => {
    ...
    return { ...item, statusCls, key: getTrackKey(item.t) };
  });
  ...
  const reconcile = () => reconcileHistoryDOM(targetItems);
  ...
  els.historyList.classList.remove("is-loading");
  animateHistoryHeight(isPanelOpen, prevHeight);
